...
type Order = {
  ...
  contract?: {
    ...
    status: string;
    body: string;
    signedBuyerAt: string | null;
    signedSellerAt: string | null;
  } | null;
...
export function OrdersPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Pedidos"
        subtitle="Do acordo ao pagamento — com estados reais."
      />
      {q.isLoading ? (
        <Spinner />
      ) : !q.data?.length ? (
        <EmptyState
          title="Sem pedidos"
          hint="Converta uma negociação aceite em contrato."
        />
      ) : (
...
export function OrderDetailPage() {
  ...
  async function ship() {
    try {
      const s = await api.post<{ id: string }>("/shipments", {
        orderId: o.id,
      ...
      toast.error(
        e instanceof Error ? e.message : "Não foi possível solicitar a entrega",
      );
    }
  }
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title={o.code}
        subtitle={`${o.productName} · ${o.quantity} ${o.unit}`}
      />
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <p className="text-xs">Total</p>
          <p className="font-display text-2xl">{kz(o.totalAmount)}</p>
        </Card>
        <Card>
        ...
        <Card>
          ...
          <p>
            {o.payment?.status ?? "Pendente"} · {o.payment?.method}
          </p>
          <p className="mt-2 text-xs text-forest-700/60">
            Métodos futuros: transferência, carteiras, mobile money, escrow via
            parceiro. Não armazenamos dados financeiros sensíveis.
          </p>
        </Card>
      </div>
      <Card>
        <h2 className="font-display text-xl">Contrato {o.contract?.code}</h2>
        <pre className="mt-3 whitespace-pre-wrap rounded-xl bg-cream-100 p-4 text-sm">
          {o.contract?.body}
        </pre>
        <div className="mt-3 flex gap-2">
          <Button variant="outline" onClick={() => void sign()}>
            Assinar
          </Button>
          <Button
            onClick={() => {
              const blob = new Blob([o.contract?.body ?? ""], {
                type: "text/plain",
              });
      ...
      <Card>
        ...
        <p className="mt-2 text-sm text-forest-700/70">
          Solicitar, escolher transportador, comparar e acompanhar.
        </p>
        {!o.shipment && (
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {!o.shipment && (
            <Button variant="gold" onClick={() => void ship()}>
              Solicitar transporte
            </Button>
          )}
          {o.shipment && (
            <Link to={`/app/logistica/${o.shipment.id}`}>
              <Button>Acompanhar envio ({o.shipment.status})</Button>
            </Link>
          )}
