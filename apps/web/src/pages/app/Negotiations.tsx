...
export function NegotiationsPage() {
  const q = useQuery({
    queryKey: ["negs"],
    queryFn: () => api.get<Neg[]>("/negotiations"),
  });
  return (
    <div>
      <PageHeader
        title="Negociações"
        subtitle="Propostas, contrapropostas e conversão em contrato."
      />
      {q.isLoading ? (
        <Spinner />
      ) : !q.data?.length ? (
        <EmptyState
          ...
          hint="Peça uma cotação a partir de uma oferta ou procura."
        />
      ) : (
              ...
              <Card className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="font-semibold">
                    {n.listing?.product.name ?? "Negociação"} · {n.quantity}{" "}
                    {n.unit}
                  </p>
                  <p className="text-sm text-forest-700/70">
                    {n.buyer.name} ↔ {n.seller.name} · {kz(n.pricePerUnit)}
                  </p>
                </div>
...
export function NegotiationDetailPage() {
  ...
  const q = useQuery({
    queryKey: ["neg", id],
    queryFn: () => api.get<Neg>(`/negotiations/${id}`),
  });
  const summary = useQuery({
    queryKey: ["neg-sum", id],
    queryFn: () =>
      api.get<Record<string, unknown>>(`/negotiations/${id}/summary`),
  });
  ...
  async function reject() {
    try {
      ...
      toast.error(e instanceof Error ? e.message : "Não foi possível recusar");
    }
  }
  async function convert() {
    try {
      const order = await api.post<{ id: string }>(
        `/orders/from-negotiation/${id}`,
      );
      toast.success("Pedido e contrato criados");
      ...
      <Card>
        <div className="flex items-center justify-between">
          <h1 className="font-display text-3xl">
            {n.listing?.product.name ?? "Negociação"}
          </h1>
          <Badge>{n.status}</Badge>
        </div>
        <p className="mt-2 text-sm">
          {n.quantity} {n.unit} · {kz(n.pricePerUnit, n.unit)} ·{" "}
          {n.deliveryPlace ?? "entrega a definir"}
        </p>
        <ol className="mt-6 space-y-3">
          {n.messages.map((m) => (
            <li key={m.id} className="rounded-xl bg-cream-100 p-3 text-sm">
              <p className="text-xs text-forest-700/50">
                {m.kind} · {new Date(m.createdAt).toLocaleString("pt-AO")}
              </p>
              <p>{m.body}</p>
            </li>
          ))}
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                <Button type="submit" disabled={counter.isPending}>
                  {counter.isPending ? "A enviar…" : "Contraproposta"}
                </Button>
                <Button
                  type="button"
                  variant="outline"
